...
import { Check, ChevronLeft, Flame, Plus, Settings, Sparkles, X } from "lucide-react";
...
const usage: Usage[] = [
  ...
  { name: "ChatGPT", used: 68, limit: 100, tone: "green" },
];
...
function App() {
  ...
  return <main className="shell">
    ...
    <section className="hero card">
      <div className="streak-copy"><span className="eyebrow">הרצף שלך</span><strong><Flame size={21} fill="currentColor" /> 12 ימים</strong><small>היום ממשיכים קדימה</small></div>
    ...
    <section className="usage-grid">
      {usage.map((metric) => { const percentage = Math.round(metric.used / metric.limit * 100); return <article className={`usage card ${metric.tone}`} key={metric.name}>
        <div className="usage-heading"><span className="provider-icon">{metric.name === "Claude" ? "AI" : <Sparkles size={15} />}</span><div><b>{metric.name}</b><small>מכסה חודשית</small></div><strong>{percentage}%</strong></div>
    ...
    <section className="todos card">
      ...
      {adding && <form className="add-form" onSubmit={(event) => { event.preventDefault(); addTodo(); }}><input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} placeholder="שם המשימה..." /><button>הוספה</button></form>}
    ...
    <footer><span><i className="online" /> הכל מעודכן</span><button><Settings size={16} /> הגדרות</button></footer>
  </main>;
}
